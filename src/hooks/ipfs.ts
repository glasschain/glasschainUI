...
async function uploadCompanyData(
  ...
  const storeResp = await lighthouse.uploadText(
...
async function getReview(commentHash: string) {
  try {
    const response = await fetch(
      `https://gateway.lighthouse.storage/ipfs/${commentHash}/`
    );
...
async function getCompanyData(companyHash: string): Promise<{
  ...
  try {
    const response = await fetch(
      `https://gateway.lighthouse.storage/ipfs/${companyHash}/`
    );

    if (!response.ok) {
      throw new Error(
        `Failed to fetch company data. Status: ${response.status}`
      );
    }
...
export { uploadReview, uploadCompanyData, getReview, getCompanyData };
